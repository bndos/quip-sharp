...
parser.add_argument("--base_model", default="nvidia/NV-Embed-v2", type=str)
parser.add_argument("--save_path", default="hessians/NV-Embed-v2", type=str)
...
def forward_layer(layer, position_ids, attention_mask, bs, device, in_q, out_q):
    ...
    while True:
        ...
        if dev_emb is None:
            ...
            out_q.put(
                {"qkv": done_qkv(), "o": done_o(), "up": done_up(), "down": done_down()}
            )
            return
...
def main(args):
    ...
    if os.path.isfile(f"{args.save_path}/dev_activations.pt"):
        ...
        dev_emb = model.embedding_model.embed_tokens(devset.to(model.device))
        after_layer = -1
    ...
    position_ids = torch.arange(args.ctx_size, dtype=torch.int64)[
        None, :
    ...
    for transformer_layer_index in range(len(model.embedding_model.layers)):
        ...
        transformer_layer = model.embedding_model.layers[transformer_layer_index]
        ...
        model.embedding_model.layers[transformer_layer_index] = None
        utils.clean()

        if args.save_activations and (
            transformer_layer_index % args.act_save_rate == 0
            or transformer_layer_index == len(model.embedding_model.layers) - 1
        ):
            if args.scratch_path is not None:
